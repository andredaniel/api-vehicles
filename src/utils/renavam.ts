...
export class Renavam {
  public static isValid(renavam: string): boolean {
    ...
    // renavam = 00639884962
    renavam = Renavam.strPad(renavam, 11, '0', 'STR_PAD_LEFT')
    ...
    var multiplicador = 2
    for (var i = 0; i < 10; i++) {
      let algarismo: number = Number(
        renavamReversoSemDigito.substring(i, i + 1)
      )
  ...
  private static strPad(
    input: string,
    pad_length: number,
    pad_string: string | undefined,
    pad_type: string
  ) {
    ...
    var str_pad_repeater = function (s: string, len: number) {
      var collect = '',
        i
